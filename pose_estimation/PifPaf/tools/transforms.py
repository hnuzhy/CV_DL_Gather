...
# configs and parameters in data.py
COCO_KEYPOINTS = [
    'nose',            # 1
    'left_eye',        # 2
    'right_eye',       # 3
...
class ImageList(torch.utils.data.Dataset):
    def __init__(self, image_paths, input_scale, preprocess=None):
        self.image_paths = image_paths
        self.input_scale = input_scale
        self.preprocess = preprocess or EVAL_TRANSFORM
    ...
    def __getitem__(self, index):
        ...
        with open(image_path, 'rb') as f:
            ...
            (w, h) = image.size
            image = image.resize((int(w*self.input_scale), int(h*self.input_scale)), Image.BICUBIC)
