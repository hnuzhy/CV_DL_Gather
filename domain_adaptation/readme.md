...
`Domain Adaptation` belongs to `Semi` or `Un-supervised Learning` / `Transfer Learning` / `Few-shot Learning`. We especially fouce on domain adaptative object detection for building robust object detection methods in real application.
